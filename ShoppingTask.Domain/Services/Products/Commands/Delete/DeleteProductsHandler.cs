...
public class DeleteProductsHandler(IUnitOfWork unitOfWork, IFileServices fileServices)
    : IRequestHandler<DeleteProductsRequest, Result>
{
    ...
    )
    {
        try
        {
            var entities = await _unitOfWork
                .Products.Find(p => request.ProductIds.Contains(p.Id))
