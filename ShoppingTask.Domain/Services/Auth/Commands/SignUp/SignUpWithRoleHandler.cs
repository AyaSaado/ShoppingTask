...
public class SignUpWithRoleHandler(IUnitOfWork unitOfWork)
    : IRequestHandler<SignUpWithRoleRequest, Result>
{
    private readonly IUnitOfWork _unitOfWork = unitOfWork;

    public async Task<Result> Handle(
        SignUpWithRoleRequest request,
        CancellationToken cancellationToken
    )
    {
        try
        {
            var userUniqueWithUserName = await _unitOfWork
                .Users.GetOneAsync(u => u.UserName == request.SignUpRequest.UserName)
            ...
            var userUniqueWithEmail = await _unitOfWork
                .Users.GetOneAsync(u => u.Email == request.SignUpRequest.Email)
                .FirstOrDefaultAsync(cancellationToken);
            ...
            var IsAdded = await _unitOfWork.Users.AddWithRole(
                user,
                request.Role.ToString(),
                request.SignUpRequest.Password
            );
